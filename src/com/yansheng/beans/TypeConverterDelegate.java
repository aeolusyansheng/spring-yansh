...
public class TypeConverterDelegate {
	...
	private Object attemptToConvertStringToEnum(Class<?> requiresType, String trimedValue,
			Object currentConvertedValue) {
		Object convertedValue = currentConvertedValue;
		if (Enum.class.equals(requiresType)) {
			int index = trimedValue.lastIndexOf(".");
			if (index > -1) {
				String enumType = trimedValue.substring(0, index);
				String fieldName = trimedValue.substring(index + 1);
				...
				try {
					...
					if (logger.isTraceEnabled()) {
						logger.trace("Enum class [" + enumType + "] cannot be loaded from [" + loader + "]", e);
					}
				} catch (Throwable e) {
					if (logger.isTraceEnabled()) {
		...
		if (convertedValue == currentConvertedValue) {
			try {
				Field enumField = requiresType.getField(trimedValue);
				convertedValue = enumField.get(null);
			} catch (Throwable e) {
				if (logger.isTraceEnabled()) {
					logger.trace("Field [" + convertedValue + "] isn't an enum value", e);
